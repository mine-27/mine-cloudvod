...
class PostType
{
    ...
    public function __construct()
    {
        ...
        add_action( "save_post_{$this->post_type}", [$this, "set_default_title"], 10, 2 );
    }
    public function set_default_title( $post_id, $post ){
        if( empty( $post->post_title ) ){
            $post->post_title = "MineCloudvod#{$post_id}";
            wp_update_post($post);
        }
    }
...
?>
        <label for="mcv_sort_no">
            <?php esc_html_e('Enter a number, which is order by ASC.', 'mine-cloudvod'); ?>
        </label>
        <p>
...
<?php
    ...
    {
        $nonce_name   = isset($_POST['mcv_sort_no_nonce']) ? sanitize_text_field(wp_unslash($_POST['mcv_sort_no_nonce'])) : '';
        ...
        $mcv_sort_no = (int) sanitize_text_field(wp_unslash($_POST['mcv_sort_no']??[]));
    ...
    public function postColumns($defaults)
    {
        ...
        unset($columns['date']);
        $columns['date'] = $v;
        if (isset($_GET['mcv_video_tag'])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- 只读判断是否展示排序列
    ...
    public function handleBulkSortNo($sendback, $doaction, $items)
    {
        if ($doaction == 'update_sort_no') {
            foreach ($items as $pid) {
                $mcv_sort_no = (int) sanitize_text_field(wp_unslash($_GET['mcv_sort_no'][$pid]??[])); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- WP bulk action 流程已在核心层校验 nonce
    ...
    public function postColumnsContent($column_name, $post_ID)
    {
        ...
        if ('sort_no' === $column_name) {
            ...
            echo '<input type="text" value="' . esc_attr($sort_no) . '" name="mcv_sort_no[' . esc_attr($post_ID) . ']" data-id="' . esc_attr($post_ID) . '" class="mcv_sort_no" style="width: 100%;text-align: right;" />';
        }
    }
        ...
        if (!empty($block_editor_content->post->post_type)) {
            if ($block_editor_content->post->post_type === $this->post_type) {
                return [
                    ...
                    'mine-cloudvod/audioplayer',
                    'mine-cloudvod/dplayer',
                    'mine-cloudvod/doge',
                    'mine-cloudvod/qiniu',
                    'mine-cloudvod/bunny',
                    'mine-cloudvod/cloudflare',
                    'mine-cloudvod/huawei-vod',
                ];
            }
        }
    ...
    public function init()
    {
        add_action('admin_enqueue_scripts',    function ($hook) {
            ...
            if( $current_screen->post_type == 'mcv_video' ){
                wp_enqueue_script('jquery');
                wp_enqueue_script('clipboard');
                wp_add_inline_script('clipboard', 'jQuery(function(){var clipboard = new ClipboardJS(".mcv_copy");clipboard.on("success", function(e) {var cur = jQuery(".mcv_copy[data-clipboard-text=\'"+e.text+"\']");cur.css("color","green");cur.after("<span>' . esc_html__('Copied', 'mine-cloudvod') . '</span>");setTimeout(function() {cur.next("span").remove();}, 2000);e.clearSelection();});});');
                wp_add_inline_script('jquery', 'jQuery(function(){
                    jQuery(".mcv_sort_no").change(function(){
        ...
        register_taxonomy('mcv_video_tag', $this->post_type, [
            'labels'                => array(
                'name'                     => _x('CloudVod Album', 'post type general name', 'mine-cloudvod'),
                'singular_name'            => _x('CloudVod Album', 'post type singular name', 'mine-cloudvod'),
                'search_items'             => _x('Search CloudVod Album', 'admin menu', 'mine-cloudvod'),
                'popular_items'            => _x('Popular CloudVod Album', 'add new on admin bar', 'mine-cloudvod'),
            ),
            'label' => __('Album', 'mine-cloudvod'),
            'public' => false,
            'show_ui' => true,
            'show_in_rest' => true,
    ...
    public function tagFilter()
    {
        ...
        $selected      = isset($_GET[$taxonomy]) ? sanitize_text_field(wp_unslash($_GET[$taxonomy])) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- 后台分类筛选下拉只读
    ...
    public function tagQuery($query)
    {
        ...
        if (isset($q_vars['orderby']) && $q_vars['orderby'] == 'sort_no') {
            $q_vars['orderby'] = ['meta_value_num' => $q_vars['order']];
            $q_vars['meta_key'] = 'sort_no'; // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- 按排序号排序，业务必需
        }
    }
    ...
    public function keep_taxonomy_menu_open($parent_file) {
        ...
        if ( $taxonomy == 'mcv_video' || $taxonomy == 'mcv_video_tag'  )
