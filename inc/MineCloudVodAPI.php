...
class MineCloudVodAPI {
	private $_url = 'https://testmcv.mine27.cn/index.php';
	...
	public function __construct(){
		...
		if(!MINECLOUDVOD_SETTINGS || !isset(MINECLOUDVOD_SETTINGS['siteid']) || !isset(MINECLOUDVOD_SETTINGS['secret']) || empty(MINECLOUDVOD_SETTINGS['siteid']) || empty(MINECLOUDVOD_SETTINGS['secret'])){
			...
			$sinfo = $this->call('register', array(
				...
				"homeurl" 	=> home_url(),
				"host" 		=> isset($_SERVER['HTTP_HOST'])?sanitize_text_field(wp_unslash($_SERVER['HTTP_HOST'])):get_site_url(),
			...
			if(isset($sinfo['siteid']) && isset($sinfo['secret'])){
				...
				$setting['endtime'] = wp_date('Y-m-d h:i:s', intval($sinfo['endtime']));
				$setting['aliplayerconfig'] = [
					'autoplay'	=> '',
					'preload'	=> '',
					'rePlay'	=> '',
					'controlBarVisibility'	=> 'hover',
					'bigPlayButton'	=> '1',
					'bigPlayButtonPosition'	=> 'ct',
					'language'	=> 'zh-cn',
					'progress'	=> '1',
					'playButton'	=> '1',
					'timeDisplay'	=> '1',
				...
				$setting['aliplayercss'] = '.prism-player{
	width: 100%;
	height: auto;
	padding-top: 56.25%;
}';
				$setting['tcplayerconfig'] = [
					'bigPlayButton' => '1',
					'controls' => '1',
					'playToggle' => '1',
					'progressControl' => '1',
					...
					'fullscreenToggle' => '1',
					'QualitySwitcherMenuButton' => '1',
				];
				$setting['tcplayercss'] = '.video-js{
	width: 100%;
	height: auto;
	padding-top: 56.25%;
}';
	...
	private function _args( $args, $sign = true ) {
		...
		elseif(($args['mode'] == 'dogevod' || $args['mode'] == 'dogeoss')
		&& isset(MINECLOUDVOD_SETTINGS['dogecloud'])){
			$args['sdk'] = $this->encrypt(MINECLOUDVOD_SETTINGS['dogecloud']);
		}
		if ( $sign ) {
