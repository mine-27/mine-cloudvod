...
class McvVideoWidget extends Widget_Base
{
    ...
    {
        return ['video', 'dplayer', 'player', 'embed', 'youtube', 'vimeo', 'bilibili', 'youku', 'aliyun', 'tencent', 'vod'];
    }
    ...
    protected function render()
    {
        ...
        $overrides = [];
        $render_safe = $video->renderBlock($overrides);
        if ($render_safe) {
            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- renderBlock 渲染的可信区块 HTML
            echo $render_safe;
            return;
        }
        ...
        if( $video ){
            $v_safe = $video->renderBlock($overrides);
            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- renderBlock 渲染的可信区块 HTML
            echo $v_safe;
        }
    }
