 ...
 *
 */
if ( ! class_exists( 'MCSF_Field_typography' ) ) {
  class MCSF_Field_typography extends MCSF_Fields {
      ...
      echo '<div class="csf--typography'. esc_attr( $chosen_class ) .'" data-depend-id="'. esc_attr( $this->field['id'] ) .'" data-unit="'. esc_attr( $args['unit'] ) .'" data-line-height-unit="'. esc_attr( $line_height_unit ) .'" data-exclude="'. esc_attr( $args['exclude'] ) .'">';
        ...
        echo '<div class="csf--blocks csf--blocks-selects">';
          ...
          if ( ! empty( $args['font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Family', 'mine-cloudvod' ) .'</div>';
            echo $this->create_select( array( $this->value['font-family'] => $this->value['font-family'] ), 'font-family', esc_html__( 'Select a font', 'mine-cloudvod' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['backup_font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Backup Font Family', 'mine-cloudvod' ) .'</div>';
            echo $this->create_select( apply_filters( 'csf_field_typography_backup_font_family', array(
              ...
              'Georgia, serif',
              'Palatino Linotype'
            ) ), 'backup-font-family', esc_html__( 'Default', 'mine-cloudvod' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_weight'] ) || ! empty( $args['font_style'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Style', 'mine-cloudvod') .'</div>';
            echo '<select class="csf--font-style-select" data-placeholder="Default">';
            echo '<option value="">'. ( ! $this->chosen ? esc_html__( 'Default', 'mine-cloudvod' ) : '' ) .'</option>';
            ...
            if ( ! empty( $args['extra_styles'] ) ) {
              ...
              echo ( ! $this->chosen ) ? '<div class="csf--title">'. esc_html__( 'Load Extra Styles', 'mine-cloudvod' ) .'</div>' : '';
              $placeholder = ( $this->chosen ) ? esc_html__( 'Load Extra Styles', 'mine-cloudvod' ) : esc_html__( 'Default', 'mine-cloudvod' );
          ...
          if ( ! empty( $args['subset'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Subset', 'mine-cloudvod' ) .'</div>';
            $subset = ( is_array( $this->value['subset'] ) ) ? $this->value['subset'] : array_filter( (array) $this->value['subset'] );
            echo $this->create_select( $subset, 'subset', esc_html__( 'Default', 'mine-cloudvod' ), $args['multi_subset'] );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_align'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Align', 'mine-cloudvod' ) .'</div>';
            echo $this->create_select( array(
              'inherit' => esc_html__( 'Inherit', 'mine-cloudvod' ),
              'left'    => esc_html__( 'Left', 'mine-cloudvod' ),
              'center'  => esc_html__( 'Center', 'mine-cloudvod' ),
              'right'   => esc_html__( 'Right', 'mine-cloudvod' ),
              'justify' => esc_html__( 'Justify', 'mine-cloudvod' ),
              'initial' => esc_html__( 'Initial', 'mine-cloudvod' )
            ), 'text-align', esc_html__( 'Default', 'mine-cloudvod' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_variant'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Variant', 'mine-cloudvod' ) .'</div>';
            echo $this->create_select( array(
              'normal'         => esc_html__( 'Normal', 'mine-cloudvod' ),
              'small-caps'     => esc_html__( 'Small Caps', 'mine-cloudvod' ),
              'all-small-caps' => esc_html__( 'All Small Caps', 'mine-cloudvod' )
            ), 'font-variant', esc_html__( 'Default', 'mine-cloudvod' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_transform'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Transform', 'mine-cloudvod' ) .'</div>';
            echo $this->create_select( array(
              'none'       => esc_html__( 'None', 'mine-cloudvod' ),
              'capitalize' => esc_html__( 'Capitalize', 'mine-cloudvod' ),
              'uppercase'  => esc_html__( 'Uppercase', 'mine-cloudvod' ),
              'lowercase'  => esc_html__( 'Lowercase', 'mine-cloudvod' )
            ), 'text-transform', esc_html__( 'Default', 'mine-cloudvod' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_decoration'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Decoration', 'mine-cloudvod' ) .'</div>';
            echo $this->create_select( array(
              'none'               => esc_html__( 'None', 'mine-cloudvod' ),
              'underline'          => esc_html__( 'Solid', 'mine-cloudvod' ),
              'underline double'   => esc_html__( 'Double', 'mine-cloudvod' ),
              'underline dotted'   => esc_html__( 'Dotted', 'mine-cloudvod' ),
              'underline dashed'   => esc_html__( 'Dashed', 'mine-cloudvod' ),
              'underline wavy'     => esc_html__( 'Wavy', 'mine-cloudvod' ),
              'underline overline' => esc_html__( 'Overline', 'mine-cloudvod' ),
              'line-through'       => esc_html__( 'Line-through', 'mine-cloudvod' )
            ), 'text-decoration', esc_html__( 'Default', 'mine-cloudvod' ) );
            echo '</div>';
          }
        ...
        echo '<div class="csf--blocks csf--blocks-inputs">';
          ...
          if ( ! empty( $args['font_size'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Size', 'mine-cloudvod' ) .'</div>';
          ...
          if ( ! empty( $args['line_height'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Line Height', 'mine-cloudvod' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['letter_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Letter Spacing', 'mine-cloudvod' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['word_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Word Spacing', 'mine-cloudvod' ) .'</div>';
            echo '<div class="csf--input-wrap">';
        ...
        if ( ! empty( $args['color'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Font Color', 'mine-cloudvod' ) .'</div>';
        ...
        if ( ! empty( $args['custom_style'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Custom Style', 'mine-cloudvod' ) .'</div>';
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'csf-webfontloader' ) ) {
        ...
        MCSF::include_plugin_file( 'fields/typography/google-fonts.php' );

        // wp_enqueue_script( 'csf-webfontloader', 'https://cdn.jsdelivr.net/npm/webfontloader@1.6.28/webfontloader.min.js', array( 'mine-cloudvod' ), '1.6.28', true );
        ...
        if ( ! empty( $customwebfonts ) ) {
          $webfonts['custom'] = array(
            'label' => esc_html__( 'Custom Web Fonts', 'mine-cloudvod' ),
            'fonts' => $customwebfonts
          );
        }

        $webfonts['safe'] = array(
          'label' => esc_html__( 'Safe Web Fonts', 'mine-cloudvod' ),
        ...
        $webfonts['google'] = array(
          'label' => esc_html__( 'Google Web Fonts', 'mine-cloudvod' ),
        ...
        wp_localize_script( 'mine-cloudvod', 'csf_typography_json', array(
          'webfonts'      => $webfonts,
      ...
      if ( $is_google ) {
        ...
        if ( $font_weight || $font_style ) {
          ...
          if ( ! empty( $style ) ) {
            ...
            MCSF::$webfonts[$method][$font_family][$style] = $style;
          }
        } else {
          MCSF::$webfonts[$method][$font_family] = array();
        }
        ...
        if ( ! empty( $this->value['extra-styles'] ) ) {
          foreach ( $this->value['extra-styles'] as $extra_style ) {
            if ( ! empty( $extra_style ) ) {
              ...
              MCSF::$webfonts[$method][$font_family][$extra_style] = $extra_style;
            }
          }
        }
