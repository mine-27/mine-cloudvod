 ...
 *
 */
if ( ! class_exists( 'MCSF_Field_select' ) ) {
  class MCSF_Field_select extends MCSF_Fields {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
            ...
            echo '<select name="'. esc_attr($field_name) .'" class="csf-hide-select hidden"'. $multiple_attr . $field_attr .'>';
          ...
          echo ( ! empty( $this->field['empty_message'] ) ) ? esc_attr( $this->field['empty_message'] ) : esc_html__( 'No data available.', 'mine-cloudvod' );
