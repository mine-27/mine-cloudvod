 ...
 *
 */
if ( ! class_exists( 'MCSF' ) ) {
  class MCSF {

    // Default constants
    public static $premium  = true;
    public static $version  = MINECLOUDVOD_VERSION;
    public static $dir      = '';
    public static $url      = '';
    public static $css      = '';
    public static $file     = '';
    ...
    public function __construct() {
      ...
      add_action( 'after_setup_theme', array( 'MCSF', 'setup' ) );
      add_action( 'init', array( 'MCSF', 'setup' ) );
      add_action( 'switch_theme', array( 'MCSF', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'MCSF', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'MCSF', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'MCSF', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'MCSF', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'MCSF_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            MCSF_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'MCSF_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            MCSF_Metabox::instance( $key, $params );
    ...
    public static function includes() {
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'MCSF_Field_'. $field ) && class_exists( 'MCSF_Fields' ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( apply_filters( 'csf_fa4', false ) ) {
        wp_enqueue_style( 'csf-fa', MINECLOUDVOD_URL.'/csf/assets/css/font-awesome.min.css', array(), '4.7.0', 'all' );
      } else {
        wp_enqueue_style( 'csf-fa5', MINECLOUDVOD_URL.'/csf/assets/css/all.min.css', array(), '5.15.3', 'all' );
        wp_enqueue_style( 'csf-fa5-v4-shims', MINECLOUDVOD_URL.'/csf/assets/css/v4-shims.min.css', array(), '5.15.3', 'all' );
      }
      ...
      wp_localize_script( 'csf', 'csf_vars', array(
        ...
        'i18n'              => array(
          'confirm'         => esc_html__( 'Are you sure?', 'mine-cloudvod' ),
          // translators: %s is characters count.
          'typing_text'     => esc_html__( 'Please enter %s or more characters', 'mine-cloudvod' ),
          'searching_text'  => esc_html__( 'Searching...', 'mine-cloudvod' ),
          'no_results_text' => esc_html__( 'No results found.', 'mine-cloudvod' ),
        ),
      ) );
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'MCSF_Field_' . $field['type'];
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field['_notice'] ) ) {
        ...
        $field            = array();
        $field['content'] = esc_html__( 'Oops! Not allowed.', 'mine-cloudvod' ) .' <strong>('. $field_type .')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      ...
      if ( ! empty( $field['dependency'] ) ) {
        ...
        $depend_escaped .= ' data-controller="'. esc_attr( $data_controller ) .'"';
        $depend_escaped .= ' data-condition="'. esc_attr( $data_condition ) .'"';
        $depend_escaped .= ' data-value="'. esc_attr( $data_value ) .'"';
        $depend_escaped .= ( ! empty( $data_global ) ) ? ' data-depend-global="true"' : '';
      ...
      echo '<div class="csf-field csf-field-'. esc_attr($field_type . $is_pseudo . $class . $visible) .'"'. $depend_escaped .'>';

      if ( ! empty( $field_type ) ) {

        if ( ! empty( $field['title'] ) ) {
          echo '<div class="csf-title">';
          echo '<h4>'. wp_kses($field['title'], mcsf_allowed_html()) .'</h4>';
          echo ( ! empty( $field['subtitle'] ) ) ? '<div class="csf-subtitle-text">'. wp_kses($field['subtitle'], mcsf_allowed_html()) .'</div>' : '';
          echo '</div>';
        }
        ...
        $classname = 'MCSF_Field_'. $field_type;

        if ( class_exists( $classname ) ) {
          $instance = new $classname( $field, $value, $unique, $where, $parent );
          $instance->render();
        } else {
          echo '<p>'. esc_html__( 'Field not found!', 'mine-cloudvod' ) .'</p>';
        }
