 ...
 *
 */
if ( ! class_exists( 'MCSF_Options' ) ) {
  class MCSF_Options extends MCSF_Abstract {
    ...
    public function ajax_save() {
      ...
      if ( ! $result ) {
        wp_send_json_error( array( 'error' => esc_html__( 'Error while saving the changes.', 'mine-cloudvod' ) ) );
      } else {
    ...
    public function set_options( $ajax = false ) {
      ...
      if ( wp_verify_nonce( $nonce, 'csf_options_nonce' ) ) {
        ...
        if ( ! $ajax && ! empty( $response[ 'csf_import_data' ] ) ) {
          ...
          $importing    = true;
          $this->notice = esc_html__( 'Settings successfully imported.', 'mine-cloudvod' );
        ...
        if ( ! empty( $transient['reset'] ) ) {
          ...
          $this->notice = esc_html__( 'Default settings restored.', 'mine-cloudvod' );
        ...
        if ( empty( $this->notice ) ) {
          $this->notice = esc_html__( 'Settings saved.', 'mine-cloudvod' );
        }
    ...
    public function add_options_html() {
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<h1>'. wp_kses($this->args['framework_title'], mcsf_allowed_html()) .'</h1>';
          echo '</div>';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'mine-cloudvod' ) .'</div>' : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( 'show all settings', 'mine-cloudvod' ) .'"><i class="fas fa-outdent"></i></div>' : '';

            echo ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="'. esc_html__( 'Search...', 'mine-cloudvod' ) .'" autocomplete="off" /></div>' : '';

            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'mine-cloudvod' ) .'" data-save="'. esc_html__( 'Saving...', 'mine-cloudvod' ) .'">';
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'mine-cloudvod' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'mine-cloudvod' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'mine-cloudvod' ) : esc_html__( 'Reset', 'mine-cloudvod' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'mine-cloudvod' ) .'">' : '';
            echo '</div>';
        ...
        echo '<div class="csf-wrapper'. esc_attr( $show_all ) .'">';
          ...
          if ( $has_nav ) {
            ...
            echo '<div class="csf-nav csf-nav-'. esc_attr( $nav_type ) .' csf-nav-options">';
              ...
              foreach ( $this->pre_tabs as $tab ) {
                ...
                $tab_error_safe = $this->error_check( $tab );
                $tab_icon_escaped  = ( ! empty( $tab['icon'] ) ) ? '<i class="csf-tab-icon '. esc_attr( $tab['icon'] ) .'"></i>' : '';

                if ( ! empty( $tab['subs'] ) ) {

                  echo '<li class="csf-tab-item">';

                    echo '<a href="#tab='. esc_attr( $tab_id ) .'" data-tab-id="'. esc_attr( $tab_id ) .'" class="csf-arrow">'. $tab_icon_escaped . wp_kses($tab['title'], mcsf_allowed_html()) . $tab_error_safe .'</a>';
                    ...
                    foreach ( $tab['subs'] as $sub ) {
                      ...
                      $sub_error_safe = $this->error_check( $sub );
                      $sub_icon_escaped  = ( ! empty( $sub['icon'] ) ) ? '<i class="csf-tab-icon '. esc_attr( $sub['icon'] ) .'"></i>' : '';

                      echo '<li><a href="#tab='. esc_attr( $sub_id ) .'" data-tab-id="'. esc_attr( $sub_id ) .'">'. $sub_icon_escaped . wp_kses($sub['title'], mcsf_allowed_html()) . $sub_error_safe .'</a></li>';
                  ...
                  echo '<li class="csf-tab-item"><a href="#tab='. esc_attr( $tab_id ) .'" data-tab-id="'. esc_attr( $tab_id ) .'">'. $tab_icon_escaped . wp_kses($tab['title'], mcsf_allowed_html()) . $tab_error_safe .'</a></li>';
          ...
          echo '<div class="csf-content">';
            ...
            foreach ( $this->pre_sections as $section ) {
              ...
              $section_icon_escaped   = ( ! empty( $section['icon'] ) ) ? '<i class="csf-section-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';
              ...
              echo ( $has_nav ) ? '<div class="csf-section-title"><h3>'. $section_icon_escaped . wp_kses($section_title, mcsf_allowed_html()) .'</h3></div>' : '';
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  MCSF::field( $field, $value, $this->unique, 'options' );

                }

              } else {

                echo '<div class="csf-no-option">'. esc_html__( 'No data available.', 'mine-cloudvod' ) .'</div>';
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'mine-cloudvod' ) .'" data-save="'. esc_html__( 'Saving...', 'mine-cloudvod' ) .'">';
          echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'mine-cloudvod' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'mine-cloudvod' ) .'">' : '';
          echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'mine-cloudvod' ) : esc_html__( 'Reset', 'mine-cloudvod' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'mine-cloudvod' ) .'">' : '';
          echo '</div>';
