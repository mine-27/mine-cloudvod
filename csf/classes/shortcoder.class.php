 ...
 *
 */
if( ! class_exists( 'MCSF_Shortcoder' ) ) {
  class MCSF_Shortcoder extends MCSF_Abstract{
    ...
    public $args         = array(
      ...
      'class'            => '',
      'gutenberg'        => array(
        'title'          => 'MCSF Shortcodes',
        'description'    => 'MCSF Shortcode Block',
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
    ...
    public function __construct( $key, $params = array() ) {
      ...
      if( ! empty( $this->args['show_in_editor'] ) ) {
        ...
        MCSF::$shortcode_instances[] = wp_parse_args( array( 'hash' => md5( $key ), 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if( MCSF::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'MCSF', 'add_admin_enqueue_scripts' ), 20 );
    ...
    ?>
      <div id="csf-modal-<?php echo $this->unique; ?>" class="wp-core-ui csf-modal hidden csf-shortcode<?php echo $single_usage . $class; ?>" data-modal-id="<?php echo $this->unique; ?>" data-nonce="<?php echo wp_create_nonce( 'csf_shortcode_nonce' ); ?>">
        ...
        if( ! empty( $section ) ) {
          ...
          if( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              MCSF::field( $field, $field_default, $shortcode, 'shortcode' );
            ...
            echo '<div class="csf--repeatable">';
              ...
              echo '<div class="csf--repeat-shortcode">';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  MCSF::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'init', array( 'MCSF_Shortcoder', 'add_guteberg_block' ) );
      }

      if ( csf_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'MCSF_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_block() {
      ...
      wp_register_script( 'csf-gutenberg-block', MCSF::include_plugin_url( 'assets/js/csf-gutenberg-block.js' ), array( 'wp-blocks', 'wp-editor', 'wp-element', 'wp-components' ) );

      wp_localize_script( 'csf-gutenberg-block', 'csf_gutenberg_blocks', MCSF::$shortcode_instances );

      foreach( MCSF::$shortcode_instances as $hash => $value ) {
