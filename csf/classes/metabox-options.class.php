 ...
 *
 */
if ( ! class_exists( 'MCSF_Metabox' ) ) {
  #[AllowDynamicProperties]
  class MCSF_Metabox extends MCSF_Abstract{
        ...
        echo '<div class="csf-wrapper'. esc_attr( $show_all ) .'">';
          ...
          if ( $has_nav ) {
            ...
            echo '<div class="csf-nav csf-nav-'. esc_attr( $nav_type ) .' csf-nav-metabox">';
              ...
              foreach ( $this->sections as $section ) {
                ...
                $tab_error_safe = ( ! empty( $errors['sections'][$tab_key] ) ) ? '<i class="csf-label-error csf-error">!</i>' : '';
                $tab_icon_escaped  = ( ! empty( $section['icon'] ) ) ? '<i class="csf-tab-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';

                echo '<li><a href="#">'. $tab_icon_escaped . wp_kses($section['title'], mcsf_allowed_html()) . $tab_error_safe .'</a></li>';
          ...
          echo '<div class="csf-content">';
            ...
            foreach ( $this->sections as $section ) {
              ...
              echo ( $section_title || $section_icon_escaped ) ? '<div class="csf-section-title"><h3>'. $section_icon_escaped . wp_kses($section_title, mcsf_allowed_html()) .'</h3></div>' : '';
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  MCSF::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );
            ...
            if ( ! empty( $this->args['show_restore'] ) || ! empty( $this->args['show_reset'] ) ) {
              ...
              echo '<span class="button csf-button-reset">'. esc_html__( 'Reset', 'mine-cloudvod' ) .'</span>';
              echo '<span class="button csf-button-cancel">'. sprintf( '<small>( %s )</small> %s', esc_html__( 'update post', 'mine-cloudvod' ), esc_html__( 'Cancel', 'mine-cloudvod' ) ) .'</span>';
              echo '</label>';
              echo '</div>';
